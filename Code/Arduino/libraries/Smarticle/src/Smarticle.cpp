...
void Smarticle::init_gait(char* msg)
{
  ...
  for (int ii=0; ii<_gait_pts[n]; ii++){
    ...
    if (_debug==1){Xbee.printf("Number:%d\tIndex:\t%d\tL:%d\tR:%d\n",n,ii,_gaitL[n][ii],_gaitR[n][ii]);}
  }
...
int Smarticle::interp_msg(char* msg)
{
  ...
  } else if (msg[1]=='G'&& msg[2]=='N'){ set_gait_num(msg); if(_debug==1){Xbee.printf("DEBUG: set gait num");}
...
} else if (msg[1]=='S'&& msg[2]=='N'){ interp_sync_noise(msg); if(_debug==1){Xbee.printf("DEBUG: set sync noise");}
  } else {
